...
public class RankingService implements Ranking_Interface {
    ...
    public RankingDTOresp create(RankingDTO rankingDTO) {
        ...
        if (member.isPresent() && competition.isPresent()) {
            ...
            EmbeddedRanking embeddedRanking = new EmbeddedRanking();
            embeddedRanking.setCompetition(competition.get());
            embeddedRanking.setMember(member.get());
            ranking.setId(embeddedRanking);
    ...
    @Override
    public RankingDTOrespNoComp get(Integer mbr, String cmp) {
        Optional<Member> member = this.memberRepository.findById(mbr);
        Optional<Competition> competition = this.competitionRepository.findById(cmp);
        EmbeddedRanking embeddedRankingID = new EmbeddedRanking();
        if (member.isPresent() && competition.isPresent()) {
            embeddedRankingID.setCompetition(competition.get());
            embeddedRankingID.setMember(member.get());
            return modelMapper.map(this.rankingRepository.findById(embeddedRankingID).get(), RankingDTOrespNoComp.class);
        }
        throw new NotFoundExeption("Member or Competition Not Found");
    }

    @Override
    public List<RankingDTOrespNoComp> getAll() {
        List<Ranking> ranking = this.rankingRepository.findAll();
        if (ranking != null) {
            return ranking.stream().map(ranking1 -> modelMapper.map(ranking1, RankingDTOrespNoComp.class)).collect(Collectors.toList());
        }
        throw new NotFoundExeption("No Ranking Found");
    }

    public List<RankingDTOrespNoComp> getRankings(String competitionCode){
        return rankingRepository.getAllByCompetition(competitionCode)
                .stream().map(ranking -> modelMapper.map(ranking,RankingDTOrespNoComp.class))
